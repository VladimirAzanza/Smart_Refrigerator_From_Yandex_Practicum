...
def add(items, title, quantity, expiration_date=None):
    ...
    items[title].append({'amount': Decimal(quantity), 'expiration_date': expiration_date}) if title in items else dict.update(items, {title: notes_list})
    return items
...
# Esta condicion hace que evite agregar elementos a nuestra lista de caducados
